...
# ------------------------------------------------------------------
# Source code from https://github.com/zuzana-masarova/cube-folding
# I added get_neighbour_ids & allocate_faces functions
# ------------------------------------------------------------------
...
def get_neighbour_ids():
    ...
    for row in range(dim_a):
        ...
        top_index_corrector = 0
        bottom_index_corrector = 0
        horizontal_index_corrector = 0
        ...
        for col in range(dim_b):
            ...
            if row > holes[0][0] + 1:
                bottom_index_corrector = -1
                horizontal_index_corrector = -1
                top_index_corrector = -1
            elif row == holes[0][0] + 1:
                bottom_index_corrector = -1
                horizontal_index_corrector = -1
                if col > holes[0][1]:
                    top_index_corrector = -1
                ...
                if col > holes[0][1]:
                    horizontal_index_corrector = -1
            elif row == holes[0][0] - 1 and col > holes[0][1]:
                bottom_index_corrector = -1
            ...
            if row == 0 or top in holes or top + [0] in slits:
                face_neighbour_ids.append(-1)
            else:
                face_neighbour_ids.append((row - 1) * dim_b + col + top_index_corrector)

            if col == 0 or left in holes or left + [1] in slits:
                face_neighbour_ids.append(-1)
            else:
                face_neighbour_ids.append(row * dim_b + col - 1 + horizontal_index_corrector)

            if row == (dim_a-1) or bottom in holes or bottom + [2] in slits:
                face_neighbour_ids.append(-1)
            else:
                face_neighbour_ids.append((row + 1) * dim_b + col + bottom_index_corrector)

            if col == (dim_b-1) or right in holes or right + [3] in slits:
                face_neighbour_ids.append(-1)
            else:
                face_neighbour_ids.append(row * dim_b + col + 1 + horizontal_index_corrector)
        
            neighbour_ids.append(face_neighbour_ids)
